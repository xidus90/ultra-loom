...
**Stand:** 2026-09-11 19:50, nach Stufe 1 des Wiki-Flottenstandards und dem
Anlauf der ulflow-Lanes; Erledigtes ins Archiv verschoben
**Haupt-Checkout:** [`master`](file:///c:/Users/micro/Documents/%23GIT/ultraloom) — Basis ist `origin/master` = `7485f63`; den Stand mit `git log --oneline origin/master..master` lesen. Eine Zahl an dieser Stelle veraltet mit dem Commit, der sie einträgt
...
Checkboxen (`- [x]`) abgehakt. **Ein ganz erledigter und gemergter Strang oder
Punkt wandert nach [Abschnitt 10, Archiv](#10-archiv).** Offene Nachläufer
eines archivierten Strangs bleiben in ihrem eigenen Abschnitt stehen.
...
Gemessen am 2026-09-11 gegen 19:40 (`git worktree list`, `git branch -a`,
`git rev-list --count`). Geschlossene Stränge stehen im Archiv.

| Strang / Fokus | Branch | Worktree-Pfad | Status | Aktiver Stand | Nächste Aktion |
|---|---|---|---|---|---|
| **Wiki-Flottenstandard** | `master` | — | 🟡 **Stufe 1 umgesetzt** | Spec `b208c36`, berichtigt in `8614c8c`: `answers.toml` wird die eine Entscheidungsdatei für ultraloom und brain, `.brain.toml` fällt daraus, ein Flottenpreset liefert die Standardantworten, vier Wiki-Arten (`brain`, `neighbour_repo`, `vault`, `none`). Stufe 0 ging am 2026-09-11 als `eb0e5f6`..`7485f63` nach master. **Stufe 1** nach `docs/.superpowers/plans/2026-09-11-wiki-flottenstandard-stufe-1.md` (`9ad8b85`): die Antwortdatei trägt `scope`, `sources` und `privacy` (`2a93460`), `ulinit` rendert daraus `.brain.toml` (`bf3b1f9`) und schreibt nie ein zweites Manifest (`63a0337`). Der Schlussreview-Fix `471eab4` (13:24) lässt einen schon deklarierten brain-Bereich seinem Manifest und trägt zwei Vorbedingungen späterer Stufen in die Spec ein (Abschnitt 6). Stufe 1 liegt auf master, **nicht** auf `origin` | Stufe 2 planen: die Hookeinträge `brain guard` und `brain wiki-gate` sowie `.agents/hooks.json` als zweiter Schreiber |
| **Agent-Harness / ulflow M1** | `feature/agent-harness` + sieben `ulflow/*` | `.worktrees/agent-harness`, `.worktrees/ulflow-{expr,flowcfg,gitwork,journal,model,runs,tmpl}` | 🟡 **in Arbeit** | **4 eigene** Commits auf `feature/agent-harness`, 0 hinter master: Entwurf (`96df882`, fünf Specs), Plan für Welle 0 und 1 (`a48a880`), Nachschliff (`51aa20b`), Verträge (`ae6e806`, 19:17). Darauf sieben Lane-Zweige mit je einem eigenen Commit, `ulflow/gitwork` mit zwei, geschrieben zwischen 19:27 und 19:37 — **zum Messzeitpunkt also mitten im Lauf**. Keiner ist in `feature/agent-harness` gemergt. Im Harness-Worktree liegen noch **zwei** ungetrackte Dateien (`2026-09-10-agent-harness-graph.html` und `-graph.standalone.html`); die Markdown-Specs, die am Vormittag nur dort lagen, sind seit `96df882` committet. Details in Abschnitt 9 | **Nicht anfassen, solange die Lanes laufen.** Weiter nach Task 8 des Plans: jede Lane prüfen, reviewen, `--no-ff` in `feature/agent-harness` mergen |
| **Mehrere LLM-Anbieter** | `feature/multi-provider-llm` (**existiert nicht mehr**) | — (Worktree am 2026-09-11 entfernt) | 🟠 **Zweig weg, Spec ungetrackt** | Am Vormittag 2026-09-11 stand hier: 155 eigene Commits, 446 hinter master, „der Zweig bleibt mit allen Commits". **Am selben Tag gegen 19:40 widerlegt:** `git branch -a` kennt den Zweig nicht, `git rev-parse --verify` scheitert, und `git reflog --all` im Hauptcheckout nennt ihn nicht. Ob die Commits noch erreichbar sind, ist **nicht gemessen**. Einzige gemessene Spur: `docs/.superpowers/specs/2026-08-24-multi-provider-llm-design.md`, ungetrackt im Hauptcheckout, mtime 2026-09-11 12:09. Wer den Zweig gelöscht hat, ist nicht belegt | Entscheiden, ob die Spec eingecheckt wird. Wird der Code noch gebraucht, zuerst klären, ob jemand die Spitze kennt |
| **Tracker-Aktualisierung mit Archiv** | `claude/offene-aufgaben-archiv` | `.worktrees/offene-aufgaben-archiv` | 🟡 **ungesichert** | Angelegt am 2026-09-11 gegen 19:50 von `master` bei `471eab4`, weil `brain guard` im orca-Worktree `wentletrap` das Schreiben dieser Datei verweigerte („outside every writable tree"). Trägt nur diese Datei, ungestaged, 0 Commits | Durchsehen, committen, nach master bringen, dann Worktree und Zweig abbauen |
| **`wentletrap`** | `update-offene-aufgaben-archiv` (vorher `wentletrap`) | `~/orca/workspaces/ultraloom/wentletrap`, **außerhalb** des Repos | 🔵 **fremdverwaltet, in Gebrauch** | Der Zweig `wentletrap` existiert nicht mehr; der Worktree hat `update-offene-aufgaben-archiv` ausgecheckt, wer umgestellt hat, ist nicht gemessen. Die Sitzung `wentletrap-fd` hat den Zweig am 2026-09-11 gegen 19:40 von `b95c187` auf `471eab4` vorgespult (Fast-Forward, 0 eigene Commits). `wentletrap-79` ist daneben offen, idle. **`brain guard` lässt in diesem Worktree nur `docs/wiki` beschreiben**, deshalb liegt die Tracker-Aktualisierung in der Zeile darüber | Nicht anfassen: orca verwaltet es. Abbauen nur über orca, oder wenn keine `wentletrap`-Sitzung mehr läuft |
| **`fix-audit-scheibe5`** | `fix-audit-scheibe5` | `~/orca/workspaces/ultraloom/fix-audit-scheibe5`, **außerhalb** des Repos | 🔵 **fremdverwaltet** | **1 eigener** Commit `02f10c3` „docs: fix incorrect placeholder in wiki/audit.md" (11:56), **11 hinter** master, nicht gemergt, Arbeitsbaum sauber. Er ersetzt den „Scheibe 5"-Satz in `docs/wiki/audit.md` durch „gefüllt wird es, sobald das Wiki Seiten hat und gepflegt wird". Die Sitzung `fix-audit-scheibe5-ae` ist seit Stunden idle | Nicht anfassen: orca verwaltet es. Den Commit übernehmen und dann den Punkt in Abschnitt 6 abhaken |
| **`fix-benchmarks-de-heading`** | `fix-benchmarks-de-heading` | `~/orca/workspaces/ultraloom/fix-benchmarks-de-heading`, **außerhalb** des Repos | 🔵 **fremdverwaltet** | **0 eigene** Commits, Spitze `e165d6d` ist Vorfahr von master, 11 dahinter, Arbeitsbaum sauber. Die Überschrift steht auf master weiter doppelt (Abschnitt 6). Die Sitzung `fix-benchmarks-de-heading-f5` ist seit Stunden idle | Nicht anfassen: orca verwaltet es |
...
## 2. Strang: Worktree-Spiegel — gemergt, Nachläufer offen
...
gemergt mit `be16705`. Die Strangzeile und die abgehakten Punkte stehen im
Archiv (Abschnitt 10); hier bleibt, was offen ist.
...
> `SessionStart` (siehe den abgehakten Punkt im Archiv, Abschnitt 10). Was
> weiter fehlt, ist die
...
Die ersten drei Zählungen am 2026-09-11 gegen 19:40 auf `471eab4` wiederholt.

- [ ] **`sameDir` liegt dreifach:** [`cmd/guard/worktree.go:520`](file:///c:/Users/micro/Documents/%23GIT/ultraloom/cmd/guard/worktree.go),
...
- [ ] **`cli` verdrahtet `os.Stdout` an fünf Stellen** ([`cmd/guard/main.go:25,45,55,70,129`](file:///c:/Users/micro/Documents/%23GIT/ultraloom/cmd/guard/main.go)),
      deshalb lässt sich der Schweigevertrag durch `cli` hindurch nicht prüfen
      („Important 4", ebenfalls herausgehalten). Es waren vier; die fünfte,
      Zeile 129 für `hook session-start`, kam mit dem Go-Hooks-Strang.
- [ ] **Der Flagset-Block steht sechsmal in `main.go`** (gezählt:
      6 × `flag.NewFlagSet`, vorher 5; der sechste kam ebenfalls mit
      `hook session-start`). Ein `subcommand()`-Helfer ist die Aufräumarbeit,
...
Die abgehakten Commits beider Schübe stehen im Archiv (Abschnitt 10).
...
> auf die Spec, die sie umgesetzt hat. Nur diese Erledigt-Vermerke sind
> ausgewertet — sie stehen abgehakt im Archiv (Abschnitt 10). **Gegen den
> heutigen Code nachgerechnet ist keiner der offenen Punkte unten**, also vor
> dem Anfassen erst prüfen, ob er noch besteht.
...
- [ ] **Ungepusht ist alles zwischen `origin/master` (`7485f63`) und `master`.**
      ...
      Der Hash `d744a0c`, der hier bis zum Vormittag 2026-09-11 stand, war
      überholt; `origin/master` steht seit dem Push von Stufe 0 auf `7485f63`.
- [ ] **`dmypy`-Daemons blockieren das Abbauen jedes Worktrees, in dem ein
      ...
      **Wird bald wieder scharf:** acht Worktrees (`agent-harness` und die
      sieben ulflow-Lanes) warten am 2026-09-11 abends auf ihren Abbau.
- [ ] **Untrackt und ungestaged im Hauptcheckout, nachgesehen 2026-09-11.**
      Drei Befunde dieses Punktes vom Vortag waren falsch oder sind überholt.
      Gegen 19:40 wiederholt: die Liste unten stimmt noch, dazu kommt die
      Multi-Provider-Spec. Der Stand jetzt:
      ...
      - `docs/wiki/log.md` zeigt als geändert, der Inhaltsdiff ist leer: nur
        Zeilenenden.
      - **Neu seit 2026-09-11 12:09:**
        `docs/.superpowers/specs/2026-08-24-multi-provider-llm-design.md`,
        die einzige gemessene Spur des gelöschten Zweigs (Abschnitt 1).
...
- [ ] **Checkout und PATH tragen verschiedene Binärstände, und seit
      ...
      Um 12:05 wurden beide von `7485f63` gebaut, also mit Stufe 0 des
      Flottenstandards, über `scripts/install.ps1` bei sauberen Go-Quellen im
      Hauptcheckout. Das Skript überschreibt ohne Sicherung; die vorigen Stände
      liegen deshalb von Hand gesichert als `ulguard.exe.old-20260911-1205` und
      `ulinit.exe.old-20260911-1205` daneben. Nach dem Neubau gemessen:
      `ulinit --detect-only` antwortet für dieses Repo `brain` / `docs/wiki/`,
      für ein `ultraloom` neben `iam_wiki` kein Wiki mehr (vorher
      `neighbour_repo` / `iam_wiki/`) und für `iam_backend` neben `iam_wiki`
      weiter `neighbour_repo` / `iam_wiki/`; `ulguard status` endet mit 0.
      **Seit 2026-09-11 13:37 laufen die beiden auseinander** — gelesen über
      `go version -m`, `vcs.revision`: `ulinit.exe` ist von `471eab4` (Ende
      Stufe 1), `ulguard.exe` weiter von `7485f63`. `cmd/guard` importiert
      `internal/answers` (`post_edit.go`, `status.go`), und Stufe 1 hat dieses
      Paket geändert. **Heute schadet das nicht:** `.ultraloom/answers.toml`
      hier trägt keinen der neuen Schlüssel `scope`, `sources`, `privacy`, und
      der Leser weist unbekannte Schlüssel nicht ab (`answers.go` kennt weder
      `Undecoded` noch eine Strenge). Beide Binäre melden `vcs.modified=true`;
      das belegt **keine** geänderten Go-Quellen, weil Go dafür
      `git status --porcelain` fragt und das die ungetrackten Katalogdateien
      mitzählt. Der Punkt bleibt offen, weil nichts die Stände zusammenhält:
      jede Änderung an `cmd/` braucht einen Neubau, und kein Gate merkt, wenn
      er fehlt.

### Zwei halbe Go-Formen in der `check`-Familie

Beim Bau von `ulinit check types` am 2026-09-10 mitgelesen; keiner der beiden
Punkte wurde angefasst — `git diff 67a9f2b master` über `cmd/init/check.go`,
`check_test.go` und `internal/commit` ist am 2026-09-11 abends leer.
...
- [ ] **Der Go-Boden ist eine Stolperdrahtgrenze, kein Ziel.** Gemessen am
      ...
      **Anheben, wenn die Luft verbraucht ist — nicht weiten.** Nach den
      Go-Hooks und Stufe 1 nicht nachgemessen.
...
## 6. Übergreifend: Das Wiki ist leer — Standard entworfen, Stufe 1 umgesetzt
...
Projekt darauf verweisen. Entschieden ist, dass das Wiki gefüllt wird, und der
Standard dafür ist in Stufe 0 und 1 gebaut (Archiv, Abschnitt 10). Seiten gibt
es noch keine.
...
- [ ] **Vor Stufe 3 (Preset): `Load` muss `bundle` prüfen** — relativ, mit
      Vorwärtsschrägstrichen. Ein Bundle mit Backslashes oder ein absolutes
      wird heute gerendert, aber brains `wiki_layout` weist es ab. Befund des
      Schlussreviews von Stufe 1, mit `471eab4` in die Spec eingetragen.
- [ ] **Vor Stufe 5 (Ausrollen): `scope` muss aus dem Verzeichnisnamen des
      Hauptcheckouts kommen**, nicht aus dem eines verlinkten Worktrees.
      `projectName` nimmt `filepath.Base` der Wurzel, ein erster Lauf in
      `.worktrees/<name>` hält also `project/<name>` fest. Schlussreview
      Stufe 1, „Important 4", ebenfalls mit `471eab4` in der Spec.
- [ ] `audit.md` verweist auf „Scheibe 5" — eine Nummerierung, die in diesem Repo
      ...
      **Der Fix liegt fertig, aber ungemergt** als `02f10c3` auf dem
      orca-Zweig `fix-audit-scheibe5` (Abschnitt 1); auf master steht der Satz
      am 2026-09-11 abends weiter in `docs/wiki/audit.md:4`.
- [ ] **`docs/benchmarks.de.md` trägt eine Überschrift doppelt.**
      ...
      liegen hintereinander. Am 2026-09-11 abends nachgezählt: weiter
      zweimal; der orca-Zweig `fix-benchmarks-de-heading` trägt noch keinen
      Commit.
...
- **26** Pläne und **31** Spezifikationen (Markdown auf master, gezählt
  2026-09-11 abends) unter
...
- **Nur auf `feature/agent-harness`** liegen fünf weitere Specs und ein Plan
  (Abschnitt 9); ungetrackt im Hauptcheckout die Multi-Provider-Spec
  (Abschnitt 5).
...
Gemergt mit `6168ef2`; die Strangzeile steht im Archiv (Abschnitt 10). Das
Schlussreview hat die Punkte unten als aufschiebbar eingeordnet; keiner hält
den Betrieb auf. Das Ausführungsledger ist mit dem Worktree abgebaut worden,
die Entscheidungen stehen in den Commit-Nachrichten des Strangs. Die Punkte
zu `status.go`, `knownLegacyHooks`, `findRoot` und `~/go/bin` sind am
2026-09-11 gegen 19:40 auf `471eab4` nachgesehen und bestehen weiter.
...
- [ ] Die Rückfallkopien `ulguard.exe.old-20260910-1407` und
      ...
      Daneben liegt ein **`ulguard.exe~`** vom 2026-09-11 10:13, gleich groß
      wie `ulguard.exe.old-20260911-1205` (5 736 960 Byte); woher er kommt, ist
      nicht gemessen.

---

## 9. Strang: ulflow — die Graph-Laufzeit in Go (M1)

* **Spezifikation (nur auf dem Zweig):** `docs/.superpowers/specs/2026-09-11-ulflow-laufzeit-design.md` — „freigegeben am 2026-09-11, nicht umgesetzt"
* **Harness-Entwurf (nur auf dem Zweig):** `2026-09-10-agent-harness-design.md`, `-graph.md`, `-intent.md`, `-knotenkatalog.md`, alle mit `96df882`
* **Durchführungsplan (nur auf dem Zweig):** `docs/.superpowers/plans/2026-09-11-ulflow-welle-0-und-1.md` (`a48a880`)

`ulflow` ist ein neues Go-Binary, das Flows ausführt und `ultraloom run`,
`show`, `resume` und `replay` aus Python ablöst. Flows werden Daten (TOML)
statt Module. M1 ist die Grundlage für den MVP `spec_to_board`; M2 sind die
Modelladapter für `claude -p` und `agy -p`. Der Plan fährt Welle 0 und 1 als
parallele Lanes, je eine in einem eigenen Worktree. Stand gemessen am
2026-09-11 gegen 19:40, während die Lanes liefen.

- [x] **Task 0, Verträge** — `ae6e806` auf `feature/agent-harness`, 19:17
- [ ] **Tasks 1–7, die sieben Lanes** `expr`, `tmpl`, `journal`, `model`,
      `gitwork`, `flowcfg`, `runs` — jede hat mindestens einen Commit
      (19:27–19:37), ob sie fertig sind, ist nicht gemessen
- [ ] **Task 8, Prüfen und Zusammenführen** — je Lane: nur eigene Dateien,
      `go test -cover` mit 100 % im Paket der Lane, zweistufiges Review,
      `merge --no-ff` in `feature/agent-harness`; danach baut das ganze Modul
- [ ] **Zwei ungetrackte Graph-HTMLs** im Worktree `agent-harness`
      (`2026-09-10-agent-harness-graph.html`, `-graph.standalone.html`):
      committen oder verschieben, bevor der Worktree abgebaut wird
- [ ] **Spec, Harness-Entwurf und Plan liegen nur auf dem Zweig.** Bis zum
      Merge nach master sieht sie aus dem Hauptcheckout niemand

---

## 10. Archiv

Hierher verschoben am 2026-09-11 aus den Abschnitten 1 bis 6: Stränge, deren
Zweig gemergt oder inhaltsgleich übernommen und abgebaut ist, und abgehakte
Punkte. Die Texte stehen unverändert bis auf Querverweise, die mit dem Umzug
nicht mehr stimmten, und bis auf kenntlich gemachte Nachsätze. **Datierte
Aussagen gelten für ihr Datum**, nicht für heute.

### Geschlossene Stränge (aus Abschnitt 1)
...
### Erledigt aus Abschnitt 2 (Worktree-Spiegel)
...
### Erledigt aus Abschnitt 3 (Dokumentation nachrechnen)
...
- [x] Befunde melden, die die Konsole nicht kodieren kann (`d0780c5` — die
      ...
      die Zeile „Konsolenkodierung" in der Strangtabelle oben)
- [x] Die Worktree-Lage datieren, auf der das Argument ruht (`8be2162` — der
      `CLAUDE.md`-Fix, erledigt aus Abschnitt 1 oben)
...
### Erledigt aus Abschnitt 4 (Backlog)

- [x] **Zeitgrenze für Prüfkommandos** (Runde 1) — umgesetzt über `process.run`
...
- [x] **`claude-agent-sdk` ist nicht gepinnt** (Runde 2) — erledigt: das Extra
      nennt `claude-agent-sdk==0.2.143`, und `run` prüft vor dem ersten Knoten,
      ob eine startbare CLI erreichbar ist.
- [x] **Eine Prüfart, ein Kommando** (`config._KINDS`, Runde 2) — erledigt,
      mehrere Kommandos je Art sind gebaut.
- [x] **Zwischen Prüfungen gibt es keine Reihenfolge** (Spec 9.4, Runde 2) —
      erledigt.

### Erledigt aus Abschnitt 5 (Repo-Zustand und Prüfketten)
...
- [x] **Der Zweigabbau ist gelaufen.** Am 2026-09-10 sind
      ...
      `feature/multi-provider-llm` und `wentletrap` — siehe Abschnitt 1.
      *Nachsatz 2026-09-11 abends: überholt. `feature/multi-provider-llm` und
      `wentletrap` gibt es nicht mehr, dazugekommen sind die sieben
      `ulflow/*`-Lanes, die orca-Zweige und `claude/offene-aufgaben-archiv`;
      der Stand steht in Abschnitt 1.*
- [x] **Worktrees aufgeräumt am 2026-09-11.** `.worktrees/go-hooks` ist nach
      ...
      `fix-benchmarks-de-heading`, siehe Abschnitt 1.
      *Nachsatz 2026-09-11 abends: „dessen Zweig bleibt" gilt nicht mehr, der
      Zweig ist weg (Abschnitt 1). Von den vier ungetrackten Specs in
      `agent-harness` sind die zwei Markdown-Dateien seit `96df882` committet.*

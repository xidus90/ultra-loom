...
1. **UltraBrain (`.ultra-brain/config.toml`, sonst `.brain.toml`):**
   ...
   * `ulinit` schreibt `.brain.toml` aus `[gates.wiki]` in `.ultraloom/answers.toml`, wenn der Modus `brain` ist und noch keine der beiden Dateien lesbar ist; ein vorhandenes Manifest bleibt, wie es ist.

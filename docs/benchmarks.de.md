...
* **Ergebnisse:**
  * Der Haupt-Checkout ohne etwas zu tun kostet **136 / 156 / 148 ms** (Median je Durchgang). Gegen eine Grundlinie von `worktree-link` auf ein Verzeichnis, das kein Repository ist — Prozessstart plus ein fehlschlagender `git`-Aufruf, `105 / 117 / 162 ms` — liegt der Sweep über acht Kandidatenverzeichnisse im Rauschen der Messung.
...
*n = 30 je Fall und Durchgang. Die Worktree-Lage von `space` während der Messung: Haupt-Checkout plus fünf registrierte Worktrees, und acht Verzeichnisse unter den beiden üblichen Elternverzeichnissen, die der Sweep abgeht.*
